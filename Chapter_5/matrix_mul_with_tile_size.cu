...
// Here is the intuition behind the tiled matrix multiplication using CUDA:
// We separate each part of the output matrix by tiles.
// Assume both input matrix have 4x4 tiles, and we want to calculate MxN
// Then, P will have 4x4 tiles, and tile P[2][3] will be the sum of:
// - M[2][1] x N[1][3]
// - M[2][2] x N[2][3]
// - M[2][3] x N[3][3]
// - M[2][4] x N[4][3]
// We will calculate the tile output of each multiplication, and add them all up
// Notice how M tile move horizontal, and N tile move vertical

// The reason why this is memory efficient is because for each input tile pair,
// we preload them into the shared memory. This is used by all threads in the
// same block.
// In fact, each thread will correspond to a cell in the output tile, and for
// each iteration, it will add the correspond value calculate using the shared
// input tiles. After that, for each iteration of tile index, we will move to a
// new tile input pair, and continue to add to the previously calculated value
// of that cell with the value calculate from the new pair.
